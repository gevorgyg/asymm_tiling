...
Args parseArgs(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--Bgenerated") {
            ...
            if (i + 1 >= argc) { std::cerr << "--trace_level requires 0|1|2\n"; exit(1); }
...
MemoryHierarchy::Parameters buildMemParams(const Config& cfg,
                                           bool b_generated, bool b_fifo)
{
...
void printStats(const MemoryHierarchy& mem, size_t cpu_cycles,
                bool b_generated, bool b_fifo)
{
...
int main(int argc, char* argv[])
{
    ...
    const std::string config_path =
        args.config_file_path.empty() ? "default.config" : args.config_file_path;
    const Config cfg = loadConfig(config_path);

    size_t cpu_cycles = 0;
    MemoryHierarchy mem(buildMemParams(cfg, args.b_generated, args.b_fifo), cpu_cycles);
    ...
    if (!args.stat_file_path.empty())
        writeStatsFile(args.stat_file_path, mem, cpu_cycles, args.b_generated, args.b_fifo);
