...
With `--3dregisters` the two `--stationary` modes are register-tiled streaming
kernels (matching the asymmetric-access-cost paper's algorithm):
- `--stationary C` prefetches the C tile and streams A subcolumns × B subrows
  as rank-1 updates (M outermost); B is re-read/regenerated each pass.
- `--stationary B` mirrors it: it prefetches (or, under `--Bsource prng_fifo`,
  generates once) one B tile and reuses it across the whole M stream while C
  accumulates across k (N outermost).

Under `--Bsource prng_fifo`, B is not stored: a small seed per B tile is read
from memory and the tile is generated by the FIFO device. The seed size is
`PRNG_FIFO_SEED_BYTES` in the config (default 8).
