...
CONFIG_PATH = os.path.join(WORKSPACE_DIR, "tests", "configs", "sweep_assoc_temp_bs.conf")
DATA_DIR = SCRIPT_DIR
...
def main():
    print("=== Running B-Stationary Cache Associativity Empirical Tiling Sweeps (16B Lines) ===")
...
def generate_report(all_results):
    ...
    with open(REPORT_PATH, "w") as f:
        f.write("# B-Stationary Cache Associativity Empirical Tiling Sweeps\n\n")
        f.write("This directory contains the results of empirical tile sweeps for a **$96 \\times 96 \\times 96$ matrix** multiplication under **B-stationary** loop ordering, sweeping cache associativity $A_1 \\in \\{1, 2, 4, 8, 16\\}$-way with 16B cache lines and 16 KB L1 size.\n\n")
        ...
        f.write("> * **L1 Cache:** 16 KB capacity, LRU replacement, Write-Back policy.\n")
        f.write("> * **L2 Cache:** 64 KB capacity, LRU replacement, Write-Back policy.\n")
        f.write("> * **DRAM Latency:** 180 cycles.\n")
        f.write("> * **Register Tile:** $4 \\times 4 \\times 4$, 8-cycle compute (`tmulac`).\n\n")
        
        f.write("## 1. Summary of Optimal Tile Shapes by Cache Associativity (B-Stationary)\n\n")
        f.write("| Cache Associativity | Precision Config | Optimal Tile Shape ($T_M \\times T_N \\times T_K$) | Ratio ($T_N/T_M$) | Footprint (KB) | L1 Hit Rate | L2 Hit Rate | DRAM Traffic (KB) | Total Cycles |\n")
        f.write("| :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |\n")
...
def generate_plots(all_results):
    ...
    try:
        ...
        assoc_keys = ["1-way", "2-way", "4-way", "8-way", "16-way"]
        ...
        for i, label in enumerate(assoc_keys):
            ax = axs[i]
            ax.grid(True, which="both", ls="--", color="#E5E5E5", zorder=0)
            
            assoc_results = all_results[label]
            
            for name in ["Symmetric Double", "Asymmetric", "Symmetric Single"]:
                results = assoc_results[name]
            ...
            ax.set_title(f"Cache Associativity = {label}", fontsize=11, fontweight='bold')
        ...
        plt.suptitle('B-Stationary Tiling Performance vs. Cache Associativity & Aspect Ratio', fontsize=14, fontweight='bold', y=0.98)
        plt.tight_layout()
        
        plot_path = os.path.join(DATA_DIR, "assoc_empirical_bstationary.png")
        plt.savefig(plot_path, bbox_inches='tight')
        plt.close()
        
        # Copy to artifact
        subprocess.run(["cp", plot_path, os.path.join(ARTIFACT_DIR, "assoc_empirical_bstationary.png")], check=True)
