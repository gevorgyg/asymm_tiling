"""C-stationary col-major α calibration (gc=0).

Measures α_calib(TM, TN) = cycles/MNK for C-stat col-major at gc=0.
Reads from the v55 C-stat col-major cache — all gc=0 entries already computed.

Col-major is used because it eliminates ghost reads, giving the true
memory-bound α for output-stationary dataflow.

Exported: get_alpha_table(base) → {l1: {(tm, tn): alpha}}
Used by multi-param-regression to supply C-stat calibration data.
"""
...
V55_CACHE = (EXPERIMENT_DIR.parent
             / "b-stationary-vs-c-stationry-col-major"
             / "results.json")
...
FLAGS = Flags(b_source="prng_fifo", stationary="output",
...
def _print_table(alpha_map: dict, l1_kb: int) -> None:
    print(f"\n  α_calib(TM, TN) = cycles/MNK   [L1={l1_kb}KB, gc=0, C-stat col-maj]")
...
def run() -> None:
    ...
    print("=" * 72)
    print("C-stationary col-major α calibration (gc=0)")
    print("=" * 72)
    print(f"M={M}, N=K={N}, TK={TK}, A_P=B_P={A_P}B, FIFO_CAP={FIFO_CAP}")
    print("(reading from v55 C-stat col-maj cache — expect all cache hits)")
