"""B-stationary vs C-stationary col-major FIFO — empirical tile shape sweep.
...
C-stat col-maj: C in %rc across all K; FIFO generates one full B tile in col-major
                order per rti; all elements are consumed (no ghost reads).
                Cost: M_reg restarts per output tile; B not reused in register.

This experiment shows whether eliminating ghost reads (via col-major ordering)
is enough for output-stationary to close the gap with B-stationary.
"""
...
def run() -> None:
    ...
    print("=" * 72)
    print("B-stationary vs C-stationary col-major FIFO")
        ...
        for gc in GC_SWEEP:
            ...
            if cy_b and cy_c:
                if cy_b <= cy_c:
                    ...
                    ratio = cy_b / cy_c
                    cell  = f"C-cmaj {ratio:.2f}× TM={tm_c},TN={tn_c}"
