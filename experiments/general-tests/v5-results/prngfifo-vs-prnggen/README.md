...
> * **Operand Precision:** Asymmetric ($A, C = 8$ bytes, $B = 2$ bytes) [except Experiment B where B precision is swept from 1B to 8B]
> * **L1 Cache:** 8 KB capacity, 4B line size, 8-way associativity, 4-cycle access, LRU replacement, Write-Back policy.
> * **L2 Cache:** 32 KB capacity, 4B line size, 8-way associativity, 15-cycle access, LRU replacement, Write-Back policy.
> * **DRAM Latency:** 180 cycles.
> * **Register Tile:** $4 \times 4 \times 4$ ($R_M \times R_N \times R_K$), 8-cycle compute.
> * **FIFO Streaming Device:** 64-entry capacity, 10-cycle generator cost (except Experiment C).
> * **Cache-Backed PRNG Generator Mode:** 8-cycle generator cost per cache line (except Experiment C).
