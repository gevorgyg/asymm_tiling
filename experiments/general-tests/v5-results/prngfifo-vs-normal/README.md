...
> * **Operand Precision:** Asymmetric ($A, C = 8$ bytes, $B = 2$ bytes) [except Sweep A where B precision is swept from 1B to 8B]
> * **L1 Cache:** 8 KB capacity, 32B line size, 6-way associativity, 4-cycle access, LRU replacement, Write-Back policy.
> * **L2 Cache:** 32 KB capacity, 64B line size, 8-way associativity, 15-cycle access, LRU replacement, Write-Back policy.
> * **DRAM Latency:** 180 cycles.
> * **Register Tile:** $4 \times 4 \times 4$ ($R_M \times R_N \times R_K$), 8-cycle compute.
> * **FIFO Streaming Device:** 64-entry capacity (except Sweep D), 10-cycle generator cost (except Sweep B).
