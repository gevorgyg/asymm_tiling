"""Associativity sweep (L1=L2), 96^3, B-stationary: best tile per assoc."""
...
TITLE = "Associativity sweep, best tile per assoc (96³, B-stationary)"
BASE_NAME = "assoc_sweep_bstat"
...
def run() -> None:
    ...
    for prec_name, prec_over in PRECISIONS:
        for assoc in ASSOC_VALUES:
            ...
            for d in duals:
                cells.append(Cell(x=math.log2(assoc), series=prec_name,
                                  traffic=d.traffic, cycles=d.cycles))
                records.append({"precision": prec_name,
                                "point": f"{assoc}-way", "d": d})
    ...
    lines.append("| metric | precision | assoc | tile (M×N×K) | value |")
    lines.append("|---|---|---|---|---|")
