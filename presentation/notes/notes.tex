...
\begin{itemize}
    \item $g_c / T_M$: Because for one block of $B$ brought into registers, its generation cost is amortized across $T_M$ rows of $A$.
    \item When $T_N$ is larger, even if $A$ leaves the cache, you need to reload it fewer times across the matrix, so the DRAM cold-fill cost is amortized.
\end{itemize}
...
\subsection*{Why the asymptote?}
\begin{itemize}
    \item Because at high $g_c$, $T_M$ is big, but $g_c$ is so big that $g_c / T_M$ is still dominant and it becomes generator-bound again. Making it so the improvement is the ratio again:
    \[
      \text{Speedup} \approx 1 - \frac{T_{M,\text{square}}}{T_{M,\text{optimal}}}
    \]
\end{itemize}

\subsection*{The place where the square and non-square are close, why is it that way?}
\begin{itemize}
    \item Because if you decrease $T_M$, the generator will stall you. If you increase $T_M$, you're at the point where it doesn't affect $\alpha$ anyway and you'll just cause $C$ to spill. So staying near that square size is just the best option.
\end{itemize}
