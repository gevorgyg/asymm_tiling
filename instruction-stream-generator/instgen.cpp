...
const Addr seed_mem_base  = 0xFFFFD000;
    ...
    if (prng) {
        ...
        for (uint tk = 0; tk < K_tiles; ++tk) {
            for (uint tj = 0; tj < N_tiles; ++tj) {
                Addr tile_offset = (tk * tile.k * B.width + tj * tile.n) * B.elem_width;
    ...
    for (uint ti = 0; ti < M_tiles; ++ti) {
        ...
        for (uint tj = 0; tj < N_tiles; ++tj) {
            ...
            for (uint tk = 0; tk < K_tiles; ++tk) {
                ...
                if (prng) {
                    // PRNG: B is generated by loading from the tile's address in the magic region
                    emit(os, "ltea", tileAddr(B, tk * tile.k, tj * tile.n), ctw, atw, B.width, B.elem_width,
                         b_id);
                } else {
